...
import { useBatteryTransactionByUser } from '@/hooks/use-battery-transaction-by-user';
...
export const CustomerTransactionTable: React.FC<
...
> = ({ userId }) => {
  ...
  const {
    ...
    transactions,
    totalCount,
  } = useBatteryTransactionByUser(userId, {
    size: TABLE_CONSTANTS.PAGINATION.ITEMS_PER_PAGE,
  });

  const handleGoBatteryTransaction = () => {
    //navigation to battery transaction page
    router.push(`/battery-transaction?searchQuery=${userId}`);
  };
